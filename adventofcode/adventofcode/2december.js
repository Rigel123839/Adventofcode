...
console.log(trym);
console.log(trym.length);

/* let z = 0;
let s = 0;
let a = 0;

for (x = 0; x < trym.length; x++) {
  s = 0;
  z = 0;

  for (b = 0; b < trym[x].length; b++) {
    let joe = trym[x].slice(b);
    console.log("Current slice:", joe);
    for (n = 0; n < trym[x].length - 1; n++) {
      let y = trym[x][n + 1] - trym[x][n];
      if (y > 0 && y <= 3) {
        s++;
      } else if (y < 0 && y >= -3) {
        s--;
      } else if (y == 0) {
        z++; // Increment z when consecutive elements are the same
      }
    }
...
/* for (x in range trym.length);
    for n in range trym[x].length;
        y = trym[x] ( n + 1 ) - trym(n);
        if 0=<y=<3:
            s+1
        if -3=<y=<0:
            s-1
    if abs(s)=length[trym[x]]:
        console.log(x + " is safe")  */

let z = 0;
let s = 0;
let a = 0;

for (let x = 0; x < trym.length; x++) {
  s = 0;
  z = 0;

  let issafe = false; // 

  for (let b = 0; b < trym[x].length; b++) {
    // remove the array peices
    let removedItem = trym[x].splice(b, 1)[0];
    console.log("Removed item:", removedItem, "Array after removal:", trym[x]);

    // check for new array
    let s = 0;
    let z = 0;

    for (let n = 0; n < trym[x].length - 1; n++) {
      let y = trym[x][n + 1] - trym[x][n];
      if (y > 0 && y <= 3) {
        s++;
      } else if (y < 0 && y >= -3) {
    ...
    if (Math.abs(s) == trym[x].length - 1) {
      issafe = true;
    }
    // restore the removed part
    trym[x].splice(b, 0, removedItem);
    console.log("Restored array:", trym[x]);
  }

  // check original array
  if (issafe || Math.abs(s) == trym[x].length - 1) {
    a++;
    console.log(x + " is safe", "s:", s, "z:", z, "length:", trym[x].length);
  } else {
    console.log(x + " is unsafe", "s:", s, "z:", z, "length:", trym[x].length);
  }
  console.log("Safe arrays count:", a);
}
